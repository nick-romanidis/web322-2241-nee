...
app.get("/add-song/:id", (req, res) => {
    ...
    if (req.session.user) {
        ...
        if (song) {
            ...
            if (found) {
                // Song was found in the cart, we already incremented qty.
                message = `The song "${song.name}" was already in the cart, added one to quantity.`;
            }
            else {
...
app.get("/check-out", (req, res) => {
    ...
    if (req.session.user) {
        ...
        if (cart.length > 0) {
            // There are items in the cart, perform the check-out.
